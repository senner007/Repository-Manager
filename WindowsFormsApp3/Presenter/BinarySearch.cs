          ...
          // Stopwatch stopwatch = Stopwatch.StartNew();
           ...
           // return newlist;
            ...
            int BinaryLowest(int left, int right)
            {
                if (left == right)
                {
                    return left >= 0 && left < listCount && list[left].LastName.StartsWith(val, MyComparer.STRINGCOMP) ? left : -1;
                }
                int mid = (right + left) / 2;

                int mylength = valLength > list[mid].LastName.Length ? list[mid].LastName.Length : valLength;

                return (string.Compare(val, list[mid].LastName.Substring(0, mylength), MyComparer.ENG, MyComparer.COMPOPTIONS) == 1) ? BinaryLowest(mid + 1, right) : BinaryLowest(left, mid);
            }
            int BinaryHighest(int left, int right)
            {
                if (left == right)
                {
                    return left - 1 >= 0 && left - 1 < listCount && list[left - 1].LastName.StartsWith(val, MyComparer.STRINGCOMP) ? left - 1 : -1;
                }
                int mid = (right + left) / 2;
                int mylength = valLength > list[mid].LastName.Length ? list[mid].LastName.Length : valLength;
                return (string.Compare(val, list[mid].LastName.Substring(0, mylength), MyComparer.ENG , MyComparer.COMPOPTIONS) == -1) ? BinaryHighest(left, mid) : BinaryHighest(mid + 1, right);
            }
        }
    }
}
